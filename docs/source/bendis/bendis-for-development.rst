...
This page gives the normal module-development workflow. More details are on the
...
4. First, initialize the repository:
   ...
   .. code-block:: text
      ...
      │   ├── .bender/            # (-) Optional Bender cache
      │   ├── Bender.yml          # (***) Editable dependency configuration
      │   ├── .bender.yml         # (***) Editable dependency overrides
      │   ├── source-policy.toml  # (***) Tracked source policy
      │   ├── .gitignore          # (***) User rules plus a Bendis-managed block
      │   ├── Bender.lock         # (-) Generated by Bendis
      │   ├── hw/                 # (-) Copied local project files, when used
      │   └── target/             # (-) Copied local project files, when used
      ├── Bender.yml              # (-) Generated configuration
      ├── .bender.yml             # (-) Generated overrides using IHEP URLs
      ├── Bender.lock             # (-) Generated exact lock file
      ├── .bender/                # (-) Materialized dependencies
      ├── hw/                     # (***) Project-local files
      ├── target/                 # (***) Project-local target files
      └── ...
   ...
   .. code-block:: yaml
      ...
      package:
        ...
        authors:
          ...
          - "Cooperators <cooperators_email@example.com>"

      dependencies:
        platform-module1: { git: "git@code.ihep.ac.cn:heris/heris-platform/platform-module1.git", version: x.y.z }
...
11. Bendis downloads dependencies from HERIS-platform and materializes them
    under ``my-module/.bender/``.

    Bendis derives the complete top-level project-local input set from the
    active ``bendis_workspace/Bender.yml`` and optional ``.bender.yml``. This
    includes local sources, include directories, path dependencies, overrides,
    plugins, external file lists, and vendor inputs. Directory names are not
    assumed; ``hw/`` and ``target/`` are mirrored only when the configuration
    references them.

    Preview the detected set without modifying the project:
    ...
    .. warning::
       ...
       Never edit copied local dependency directories under
       ``bendis_workspace/``. Changes in these directories will be lost after
       running ``bendis update``.
...
14. Continue developing the module in Vivado. Project-local files selected by
    the Bender targets, together with dependencies downloaded from
    HERIS-platform and stored under ``.bender/git/``, are linked into the
    Vivado project.
...
1. Edit project-local RTL in the root-level directories declared by the Bender
   configuration. HERIS modules commonly use ``hw/`` and ``target/``.
...
2. Carefully check whether the module contains RTL files outside the ``hw/``
   ...
   .. warning::
      ...
      Bendis supports project-local dependencies outside ``hw/`` and
      ``target/``. Keep their paths relative to the project and declare them
      in ``bendis_workspace/Bender.yml`` or
      ``bendis_workspace/.bender.yml``.
