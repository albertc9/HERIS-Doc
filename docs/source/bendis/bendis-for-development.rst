...
11. Bendis downloads dependencies from HERIS-platform and materializes them
    ...
    Bendis detects top-level directories used by local ``path`` dependencies and
    copies them from the project root into ``bendis_workspace/``. If no local
    Path dependency is configured, it uses ``hw/`` and ``target/``.
