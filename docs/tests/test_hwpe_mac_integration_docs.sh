...
grep -qF 'The recorded integration validation passes' "$ARCHITECTURE"
grep -qF 'Setup meets the gate with no positive margin.' "$ARCHITECTURE"
grep -qF 'current 2.1.4 integration has passed this' "$ARCHITECTURE"
...
grep -qF '``kcu105-console.log``' "$CI"
grep -qF '``target/fpga/kcu105/reports``' "$CI"
